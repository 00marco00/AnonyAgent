...
const MARKERS = [".anonyagent", "AGENTS.md", ".git"];
/**
...
export function loadProjectConfig(from = process.cwd()) {
    const root = findProjectRoot(from);
    if (!root)
        return null;
    // AGENTS.md may live at the root, or inside .anonyagent/.
    const agentsMd = readIfExists(join(root, ".anonyagent", "AGENTS.md")) ??
    ...
    const rules = singleRules && rulesDir
        ? `${singleRules.trim()}\n\n${rulesDir}`
...
export function projectPromptSection(cfg) {
    if (!cfg || (!cfg.agentsMd && !cfg.rules))
        return "";
    const parts = [
        `PROJECT CONTEXT`,
