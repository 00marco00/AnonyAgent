 ...
 */
export function MultilineInput({ value, cursor, onChange, onSubmit, onHistoryUp, onHistoryDown, placeholder = "", isActive = true, }) {
    useInput((char, key) => {
        ...
        if (char && char.length > 1 && !key.ctrl && !key.meta) {
            ...
            onChange(value.slice(0, cursor) + insert + value.slice(cursor), cursor + insert.length);
            return;
        }
        if (key.return) {
            onSubmit(value);
        ...
        // Plain character.
        if (char && !key.meta) {
            onChange(value.slice(0, cursor) + char + value.slice(cursor), cursor + char.length);
        }
    }, { isActive });
    // ── Render ────────────────────────────────────────────────────────────
    if (value.length === 0) {
        return (_jsx(Box, { flexGrow: 1, children: _jsxs(Text, { children: [_jsx(Text, { inverse: true, children: " " }), _jsx(Text, { color: "gray", children: placeholder })] }) }));
    }
    const before = value.slice(0, cursor);
    const at = value.charAt(cursor) || " ";
    const after = value.slice(cursor + 1);
    return (_jsx(Box, { flexGrow: 1, children: _jsxs(Text, { children: [before, _jsx(Text, { inverse: true, children: at }), after] }) }));
}
// ── Helpers ──────────────────────────────────────────────────────────────
...
function prevWordStart(value, cursor) {
    let i = cursor;
