...
import type { AgentCallbacks, AgentDeps } from "../agent/loop.js";
...
export interface ToolContext {
  /** Working directory the agent should operate inside. */
  cwd: string;
  /** Shared allocator so tool results stay coherent with the chat session. */
  allocator: PlaceholderAllocator;
  /** OpenAI tool_call_id this run was triggered by. */
  callId?: string;
  /** Agent-loop wiring; required by spawn_agent, ignored by other tools. */
  agentDeps?: AgentDeps;
  agentCallbacks?: AgentCallbacks;
  agentOpts?: { autoAllow?: boolean; maxIterations?: number };
}

export interface ToolDefinition {
  name: string;
  description: string;
  /** JSON Schema for the function's parameters (OpenAI tools format). */
  parameters: Record<string, unknown>;
  /** True for purely-observational tools that don't modify the world. */
  readOnly: boolean;
  /** Pretty-print the call for review. */
  summarize: (args: Record<string, unknown>) => string;
  /** Run the tool. Returned `output` is the raw (un-anonymized) result string. */
  run: (
...
const readFileTool: ToolDefinition = {
  name: "read_file",
  description:
    "Read a UTF-8 text file from the project. Returns the file content with line numbers.",
  parameters: {
    type: "object",
    properties: {
      path: {
  ...
  run: async (args, ctx) => {
    ...
    if (!existsSync(path)) return { output: `file not found: ${args.path}`, isError: true };
...
const writeFileTool: ToolDefinition = {
  name: "write_file",
  description:
    "Create or fully overwrite a file with the given content. Creates parent directories as needed.",
  parameters: {
    type: "object",
    properties: {
...
const editFileTool: ToolDefinition = {
  name: "edit_file",
  description:
    "Replace one occurrence of `old_string` with `new_string` in the file. `old_string` MUST be unique in the file; otherwise this errors. Use larger context if needed.",
  parameters: {
    type: "object",
    properties: {
      path: { type: "string" },
...
const listDirTool: ToolDefinition = {
  ...
  run: async (args, ctx) => {
    ...
    if (!existsSync(path)) return { output: `not found: ${args.path}`, isError: true };
    const stat = statSync(path);
    if (!stat.isDirectory()) return { output: `not a directory: ${args.path}`, isError: true };
    const entries = readdirSync(path).sort();
    const lines: string[] = [];
    for (const name of entries) {
      try {
        const s = statSync(join(path, name));
        if (s.isDirectory()) lines.push(`  ${name}/`);
        else lines.push(`  ${name}  ${s.size}b`);
      } catch {
...
const globTool: ToolDefinition = {
  name: "glob",
  description:
    "Find files by glob pattern (e.g. 'src/**/*.ts'). Returns up to 200 matches.",
  parameters: {
    type: "object",
    properties: {
      pattern: { type: "string" },
  ...
  run: async (args, ctx) => {
    ...
    const out: string[] = [];
    for await (const m of (glob as unknown as (p: string, o: { cwd: string }) => AsyncIterable<string>)(pattern, { cwd: ctx.cwd })) {
      out.push(m);
...
const grepTool: ToolDefinition = {
  name: "grep",
  description:
    "Search for a regex pattern across files. Returns matching lines (max 200).",
  parameters: {
    type: "object",
    properties: {
  ...
  run: async (args, ctx) => {
    ...
    const matchesGlob = (p: string) => {
      if (!globPat) return true;
    ...
    const visit = (p: string) => {
      ...
      if (!s.isFile()) return;
      if (!matchesGlob(p)) return;
      if (s.size > 1_000_000) return; // skip huge files
      let content: string;
      try {
...
const bashTool: ToolDefinition = {
  name: "bash",
  description:
    "Run a shell command in the project root and capture stdout+stderr. 30s timeout. Use for build/test/lint/git.",
  parameters: {
    type: "object",
    properties: {
      command: { type: "string" },
...
const spawnAgentTool: ToolDefinition = {
  name: "spawn_agent",
  description:
    "Delegate a self-contained subtask to a sub-agent that has the same tools as you and operates in the same project. Returns the sub-agent's final report. Use for parallelizable research, audits, or contained refactors — you may call this multiple times in one turn and they will run concurrently. Do NOT use it for trivial single-tool work, and do NOT pass tasks that require asking the user clarifying questions.",
  parameters: {
    type: "object",
    properties: {
      task: {
        type: "string",
        description:
          "Clear, self-contained task description. The sub-agent will not have access to the parent conversation — include all background it needs to act.",
      },
      context: {
        type: "string",
        description:
          "Optional extra context (file paths, constraints, prior findings).",
      },
    },
    required: ["task"],
  },
 ...
 */
export async function anonymizeToolOutput(
  output: string,
  allocator: PlaceholderAllocator,
): Promise<{ anonymized: string; entities: Entity[] }> {
 ...
 */
export function deanonymizeArgs(
  args: Record<string, unknown>,
  allocator: PlaceholderAllocator,
): Record<string, unknown> {
  const reverse = allocator.reverseMap();
  const sub = (s: string): string =>
    s.replace(/\[[A-Z_]+_\d+\]/g, (p) => reverse.get(p) ?? p);
